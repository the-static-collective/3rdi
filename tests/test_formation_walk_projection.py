...
from three_rdi.epistemic import compile_cut as compile_prior_cut  # noqa: E402
...
class FormationWalkProjectionTests(unittest.TestCase):
    ...
    def test_omitted_walk_family_is_byte_identical_to_prior_epistemic_compiler(self) -> None:
        field = base_field()
        field.pop("formation_walks")
        new_receipt = compile_cut(field, "a1")
        prior_receipt = compile_prior_cut(field, "a1")
        self.assertEqual(canonical_json(new_receipt), canonical_json(prior_receipt))
        self.assertEqual(new_receipt["projection_digest"], prior_receipt["projection_digest"])
