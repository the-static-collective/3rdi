...
The `formation_walks` and `withheld_formation_walks` surfaces appear only when the input field explicitly declares the optional `formation_walks` family. Fields that omit the family retain the exact legacy receipt bytes and digest.
